...
static int __init bluesleep_init(void)
{
	...
	ent = create_proc_entry("btwake", 0666, sleep_dir);
	if (ent == NULL) {
...
#if BT_BLUEDROID_SUPPORT
	/* read/write proc entries */
	ent = create_proc_entry("lpm", 0666, sleep_dir);
	if (ent == NULL) {
	...
	/* read/write proc entries */
	ent = create_proc_entry("btwrite", 0666, sleep_dir);
	if (ent == NULL) {
